...
- Next experiment: commit/push the clean authorization binding, then run the sole deterministic development evaluation from the exact pushed production evidence commit.
...
- Exact resume state: active experiment `btc-eth-volatility-managed-equal-weight-v1`; calls 2/4 used, two remain; repair 1/1 used, zero remain; cycle 0/1 used; production evidence commit `0adaa6b...` and its five hashes are authorization-bound; market values and 2026 holdout remain unopened.
